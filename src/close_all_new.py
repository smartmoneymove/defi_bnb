...
class PositionCloser:
    ...
    def __init__(self, 
        ...
        if farm_address:
            ...
            if farm_abi_path and Path(farm_abi_path).exists():
                ...
                print(f"⚠️ Файл ABI фарминга не найден: {farm_abi_path}")
        else:
            ...
            for i in range(nft_count):
                try:
                    ...
                    if (token0.lower() == self.token0_address.lower() and 
                        ...
                        print(f"  ⚠️ NFT ID {token_id} не соответствует нашему пулу")
            ...
            for i in range(farm_balance):
                try:
                    ...
                    if user_address.lower() != self.signer_address.lower():
                        print(f"      ⚠️ NFT ID {token_id} принадлежит другому пользователю")
                        continue
                    ...
                    if (token0.lower() == self.token0_address.lower() and 
                        ...
                        print(f"      ⚠️ NFT ID {token_id} в фарминге не соответствует нашему пулу")
    ...
    async def _wait_for_tokens_return(self, expected_min_value: Decimal = Decimal("10")) -> tuple[int, int, Decimal] | None:
        ...
        try:
            ...
            print(f"⚠️ Ошибка получения цены: {e}, используем fallback")
    ...
    async def _execute_remove_liquidity_multicall(self, positions_to_close: list) -> bool:
        ...
        if self.farm_address and self.farm_contract:
            ...
            for slot_id, nft_id, position_info in positions_to_close:
                # Проверяем реальное состояние NFT в блокчейне, а не кэшированные данные
        ...
        # Ждем 5 секунд для подтверждения транзакций вывода NFT
        if self.farm_address and self.farm_contract:
            print("⏳ Ожидание 5 секунд для подтверждения транзакций вывода NFT...")
            await asyncio.sleep(5)
            
            # Проверяем что все NFT действительно выведены из фарминга
            print("🔍 Проверяем статус NFT после вывода...")
            for slot_id, nft_id, position_info in positions_to_close:
                if position_info.get('location') == 'farm':
                    try:
                        # Проверяем владельца NFT - должен быть наш кошелек
                        owner = self.nonf_pos_manager.functions.ownerOf(nft_id).call()
                        if owner.lower() != self.signer_address.lower():
                            print(f"⚠️ NFT {nft_id} еще не на кошельке, ждем еще 10 секунд...")
                            await asyncio.sleep(10)
                            break
                    except Exception as e:
                        print(f"⚠️ Ошибка проверки NFT {nft_id}: {e}")
        ...
        # Пробуем multicall до 2 раз с динамическим получением цены газа
        for attempt in range(2):
            try:
                # Динамически получаем актуальную цену газа перед каждой попыткой
                base_gas_price = await self._get_gas_price()
                
                if attempt == 0:
                    # Первая попытка: используем полную цену газа
                    gas_price_to_use = base_gas_price
                    print(f"⛽ Multicall попытка #{attempt + 1} с актуальной ценой газа: {gas_price_to_use}")
                else:
                    # Вторая попытка: увеличиваем на 20% от актуальной цены
                    gas_price_to_use = int(base_gas_price * 1.2)
                    print(f"⛽ Повторная попытка multicall #{attempt + 1} с увеличенным газом: {gas_price_to_use} (+20%)")
                ...
                if ("replacement transaction underpriced" in error_msg or 
                    ...
                    "insufficient funds" in error_msg or
                    "max fee per gas less than block base fee" in error_msg) and attempt < 1:
    ...
    async def rebalance_portfolio_1_to_1(self, router_address: str) -> bool:
        ...
        if total_portfolio_value_usdc < min_portfolio_value_for_rebalance:
            print(f"⚠️ Портфель слишком мал для ребалансировки (${total_portfolio_value_usdc:.2f} < ${min_portfolio_value_for_rebalance}). Пропускаем.")
            return True
        ...
        if usdt_value_difference > 0:  # Нужно свапнуть USDT в BTCB
            ...
            if usdt_value_difference < 1:
                print("  ⚠️ Рассчитанная разница слишком мала для свапа USDT -> BTCB.")
                return True
            ...
            # Проверяем, что разница достаточно значима
            if usdt_value_difference < 1:
                print("  ⚠️ Рассчитанная разница слишком мала для свапа BTCB -> USDT.")
                return True
    ...
    async def swap_cake_to_usdc(self, router_address: str, cake_address: str) -> bool:
        ...
        if cake_balance_human < min_cake_for_swap:
            print(f"⚠️ Баланс CAKE слишком мал для свапа ({cake_balance_human:.6f} < {min_cake_for_swap})")
            return True
